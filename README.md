...
| | 特性 | 说明 |
|---|---|---|
| 📝 | **Obsidian 风格实时编辑器** | Markdown 所见即所得，点击块就地编辑原文，Esc 退出 |
...
Markdown 采用 **实时渲染 + 块级编辑**：整篇以渲染态展示，点击任意块暴露原始 Markdown 就地编辑，`Ctrl+Enter` 提交 / `Esc` 取消。支持任务列表勾选回写、`[[wikilink]]` 跳转、`⌨` 标注的代码块与圆角表格。
